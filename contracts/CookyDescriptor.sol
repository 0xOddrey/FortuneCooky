...
import  "./libraries/Lines4.sol";

contract CookyDescriptor is ICookyDescriptor {



    //Indexed list of Hexcodes 
    string[] colors = ["#F3ECCB", "#84DFD3", "#ACCCE4", "#B4BCD7", "#D3B5C7", "#CBAF2E", "#3EADB0", "#3D86BC", "#5668A2", "#9C5D83"];

    /**
     * @notice Gets the two lines of code and adds SVG formatting
     * @return a string of svg code
     */
    ...
    /**
     * @notice SVG code for actual fortune cooky
     * @return a string of svg code
     */
    ...
    /**
     * @notice pulls in three hex codes and builds background colors
     * @return a string of svg code
     */
    ...
    /**
     * @notice Calls one of four Fortune Cooky Libraries based on random seed which changes every 7 days 
    ...
    function _getFortune(uint256 tokenId, uint256 tseed, uint256 cseed) public override pure returns(string[2] memory) {
        ...
        uint scaled = seed % 80;
        if (scaled >= 61) {
            return Lines4.getFortuneLines(scaled);
        } else if (scaled >= 41) {
    ...
    /**
     * @notice Pulls all components of SVG code to build metadata 
     * @return base encoded json string 
     */
    function _buildFortuneCooky(uint256 tokenId, uint256 tseed, uint256 cseed, uint256 nextDate) public view override returns (string memory) {
        ...
        string memory background = _getBackground(colors[iOne], colors[iTwo], colors[iThree]);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '", "description": "',
                        string(abi.encodePacked(lines[0], " ", lines[1])),
                        '", "attributes": [{"display_type": "date", "trait_type": "Next Fortune",  "value": ',
                        Strings.toString(nextDate),
